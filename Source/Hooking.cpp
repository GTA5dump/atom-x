...
namespace Mod_Hub_Base
{
	...
	{
		if (std::strcmp(label, "HUD_JOINING") == 0) return "Loading GTA Online with ThunderX!";
		if (std::strcmp(label, "HUD_TRANSP") == 0) return "Modding never been easier - ThunderX!";
		if (std::strcmp(label, "PM_EXIT_GAME") == 0) return "Exit GTAV with ThunderX";
		if (std::strcmp(label, "PM_ENTER_MP") == 0) return "Play GTA Online with ThunderX!";
		if (std::strcmp(label, "PM_INF_PGOB") == 0) return "Play GTA Online with ThunderX. ThunderX is currently undetected so no need to worry, but please do not abuse money methods!";
	...
	*/
	...
	void Hooking::FindPatterns()
	{
		...
		Log::Msg("Downloading...");
		Log::Msg("Everything seems good...");
		Log::Msg(Mod_Menu_Name " has been succesfully loaded! Open with F4.");
		Log::Msg("Join our discord server: https://tabmax.lol/dc.html");
		Log::Msg("Our official website is: https://tabmax.lol/");
		///system("start https://tabmax.lol/dc.html");
